...
use super::convert::IntoAttributeValue;
use super::error::{Error, Result};
use super::namespaces::NSChoice;
use super::node::Node;
use super::prefixes::{Namespace, Prefix, Prefixes};
...
fn ns_name<NS: Into<String>>(namespace: Option<NS>) -> Option<String> {
  namespace.map(|namespace| namespace.into())
}

fn clone_ns<NS: Into<String>>(namespace: Option<NS>) -> Option<String> {
  namespace.map(|namespace| namespace.into())
}
...
impl Element {
  ...
  /// assert_eq!(bare.text(), "");
  /// ```
  pub fn bare<S: Into<String>, NS: Into<String>>(name: S, namespace: Option<NS>) -> Element {
    Element::new(
      name.into(),
      ns_name(namespace),
      None,
  ...
  pub fn has_ns<'a, NS: Into<NSChoice<'a>>>(&self, namespace: NS) -> bool {
    match &self.namespace {
      Some(self_ns) => namespace.into().compare(self_ns.as_ref()),
      None => false,
    }
  }
  ...
  pub fn write_to_inner<W: Write>(
    ...
    let self_prefix: (Option<String>, bool) = match &self.namespace {
      None => (None, true),
      Some(namespace) => {
        // Element namespace
    ...
    if let Some(namespace) = &self.namespace {
      match self_prefix {
        (Some(ref p), true) => {
          ...
          start.push_attribute((key.as_bytes(), namespace.as_bytes()));
          all_prefixes.insert(self_prefix.0, namespace.clone());
        }
        (None, true) => {
...
mod tests {
  ...
  fn test_from_reader_simple() {
    ...
    let elem2 = Element::builder("foo", Some("ns1")).build();
  ...
  fn test_from_reader_nested() {
    ...
    let nested = Element::builder("bar", Some("ns1"))
      .attr("baz", "qxx")
      .build();
    let elem2 = Element::builder("foo", Some("ns1")).append(nested).build();
