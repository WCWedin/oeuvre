...
fn build_test_tree() -> Element {
  let mut root = Element::builder("root", Some("root_ns"))
    .attr("xml:lang", "en")
    .attr("a", "b")
    .build();
  root.append_text_node("meow");
  let child = Element::builder("child", Some("root_ns"))
    .attr("c", "d")
    .build();
  root.append_child(child);
  let other_child = Element::builder("child", Some("child_ns"))
    .attr("d", "e")
    .attr("xml:lang", "fr")
    .build();
...
#[test]
fn test_real_data() {
  let correction = Element::builder("replace", Some("urn:xmpp:message-correct:0")).build();
  let body = Element::builder("body", Some("jabber:client")).build();
  let message = Element::builder("message", Some("jabber:client"))
    .append(body)
    .append(correction)
    .build();
  let stream = Element::builder("stream", Some("http://etherx.jabber.org/streams"))
    .prefix(
  ...
  let jid = Element::builder("jid", Some("urn:xmpp:presence:0")).build();
  let nick = Element::builder("nick", Some("urn:xmpp:presence:0")).build();
  let mix = Element::builder("mix", Some("urn:xmpp:presence:0"))
    .append(jid)
    .append(nick)
    .build();
  let show = Element::builder("show", Some("jabber:client")).build();
  let status = Element::builder("status", Some("jabber:client")).build();
  let presence = Element::builder("presence", Some("jabber:client"))
    .append(show)
    .append(status)
    .append(mix)
    .build();
  let item = Element::builder("item", Some("http://jabber.org/protocol/pubsub"))
    .append(presence)
    .build();
  let items = Element::builder("items", Some("http://jabber.org/protocol/pubsub"))
    .append(item)
    .build();
  let pubsub = Element::builder("pubsub", Some("http://jabber.org/protocol/pubsub"))
    .append(items)
    .build();
  let iq = Element::builder("iq", Some("jabber:client"))
    .append(pubsub)
    .build();
  let stream = Element::builder("stream", Some("http://etherx.jabber.org/streams"))
    .prefix(
...
fn writer_with_prefix() {
  let root = Element::builder("root", Some("ns1"))
...
fn writer_no_prefix_namespace() {
  let root = Element::builder("root", Some("ns1")).build();
...
fn writer_no_prefix_namespace_child() {
  let child = Element::builder("child", Some("ns1")).build();
  let root = Element::builder("root", Some("ns1")).append(child).build();
  // TODO: Same remark as `writer_no_prefix_namespace`.
  assert_eq!(String::from(&root), r#"<root xmlns="ns1"><child/></root>"#);

  let child = Element::builder("child", Some("ns2"))
    .prefix(None, "ns3")
    .unwrap()
    .build();
  let root = Element::builder("root", Some("ns1")).append(child).build();
...
fn writer_with_prefix_deduplicate() {
  let child = Element::builder("child", Some("ns1"))
    // .prefix(Some(String::from("p1")), "ns1")
    .build();
  let root = Element::builder("root", Some("ns1"))
    .prefix(Some(String::from("p1")), "ns1")
  ...
  let grandchild = Element::builder("grandchild", Some("ns1")).build();
  let child = Element::builder("child", Some("ns2"))
    .append(grandchild)
    .build();
  let root = Element::builder("root", Some("ns1")).append(child).build();
...
fn writer_escapes_text() {
  let root = Element::builder("root", Some("ns1")).append("<3").build();
  let mut writer = Vec::new();
  {
    root.write_to(&mut writer).unwrap();
  }
...
#[test]
fn builder_works() {
  let elem = Element::builder("a", Some("b"))
    .attr("c", "d")
    .append(Element::builder("child", Some("b")))
    .append("e")
    .build();
...
fn namespace_propagation_works() {
  let mut root = Element::builder("root", Some("root_ns")).build();
  let mut child = Element::bare("child", Some("root_ns"));
  let grandchild = Element::bare("grandchild", Some("root_ns"));
...
fn xml_error() {
  match "<a xmlns='ns1'></b>".parse::<Element>() {
    Err(super::error::Error::XmlError(_)) => (),
...
fn invalid_element_error() {
  match "<a:b:c>".parse::<Element>() {
    Err(super::error::Error::InvalidElement) => (),
    err => panic!("No or wrong error: {:?}", err),
  }
}
...
fn empty_namespace_parses() {
  match "<a/>".parse::<Element>() {
    Err(err) => panic!("Empty namespace threw error: {:?}", err),
    _ => (),
  }
}
